...
class PegasusCommander(Node):
    def __init__(self):
        ...
        if(run_socat()):
            print("Using socat for Arduino communication")
...
def run_socat():
    """
    Execute socat to create two linked pseudo-terminals /tmp/ttyV0 and /tmp/ttyV1.
    """
    try:
        cmd = "socat -d -d PTY,link=/tmp/ttyV0,raw,echo=0 PTY,link=/tmp/ttyV1,raw,echo=0"
        subprocess.run(cmd, shell=True, check=True, text=True)
        logging.info("Successfully executed socat command")
        portname = '/tmp/ttyV0'
        return True 
